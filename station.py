...
def get_chromatic_frame():
    """Renders high-density Pink, Yellow, and Blue stars"""
    width = 75
    center = width // 2
    # Bigger stars: ● (Blue), ✦ (Yellow), █ (Pink)
    stars = [
        (C_PINK + "█" + C_RESET), (C_YELLOW + "✦" + C_RESET), (C_BLUE + "●" + C_RESET),
        (C_PINK + "•" + C_RESET), (C_YELLOW + "★" + C_RESET), (C_BLUE + "◌" + C_RESET)
    ]
    ...
    for i in range(width):
        dist = abs(i - center)
        prob = 0.18 * (1.1 - (dist / center)) # Galactic core density
        if random.random() < (prob + 0.02):
            line.append(random.choice(stars))
        else:
            line.append(" ")
    return "".join(line)
...
def main():
    clear()
    print("\033[1;95m  SOVEREIGN NEBULA v51.0 | CHROMATIC SYNC ACTIVE\033[0m")
    pilot_id = input("\n\033[1;32m[ IDENTITY ]\033[0m CAPTAIN ID: ").strip().upper() or "AVI"
    data = load_data(pilot_id)
    
    clear()
    print("\033[1;36m[ ATMOSPHERE ]\033[0m SELECT NEURAL STREAM (AUTOPLAY):")
    ...
    clear()
    print(f"\033[1;33m[ MISSION ]\033[0m GOAL FOR {pilot_id}:")
    goal = input(">> ")
    ...
    while True:
        ...
        print("\n [1] ENGAGE_WARP [2] HISTORY_LOG [3] IROH [4] EXIT")
        
        cmd = input("\n>> ")
        
        if cmd == "1":
            duration = 25 * 60
            field = [get_chromatic_frame() for _ in range(14)]
            try:
                while duration > 0:
                    m, s = divmod(int(duration), 60)
                    clear()
                    print(f"\033[1;95m{'═'*22} CHROMATIC_WARP_ACTIVE {'═'*22}\033[0m")
                    field.pop(0); field.append(get_chromatic_frame())
                ...
                print("\n\033[1;32m✔ MISSION COMPLETE. SYNCING LOGS & REFRESHING STREAM...\033[0m")
            ...
            print(f"PILOT_LOG_2026_SYNC: {datetime.now().strftime('%Y-%m-%d')}")
            for h in data['history'][-10:]: print(f" [{h['ts']}] {h['task']} ({h['vibe']})")
            input("\n[ENTER TO RETURN]")
        elif cmd == "3":
            clear(); print(f"\n\033[1;36mUncle Iroh: \"{random.choice(IROH_QUOTES)}\"\033[0m"); input("\n[BACK]")
        elif cmd == "4":
